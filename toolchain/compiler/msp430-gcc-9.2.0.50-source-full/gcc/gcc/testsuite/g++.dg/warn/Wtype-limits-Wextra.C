...
void b (unsigned short x)
{
  ...
  /* { dg-warning "comparison of unsigned expression < 0 is always false" "" { target short_eq_int } .-1 } */
  if (x >= 0) return;/* { dg-warning "comparison is always true due to limited range of data type" "" { target { ! short_eq_int } } } */
  /* { dg-warning "comparison of unsigned expression >= 0 is always true" "" { target short_eq_int } .-1 } */

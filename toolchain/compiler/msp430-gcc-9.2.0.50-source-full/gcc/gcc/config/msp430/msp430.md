...
; to what extent can we rely on the value in op0 register defo being a QI with
; high bits set to 0? MODES_TIEABLE_P seems to confirm that we can do this
...
;; We started with: 
;; (set (reg:SI)
;;    (ashift:SI (zero_extend:SI (mem/c:QI (symbol_ref:PSI)))
;;        (const_int 2)))
;; FIXME A post_inc in op1 causes duff-{2,4} to ICE so we dissallow that for now.
(define_insn ""
  [(set (match_operand:SI 0 "register_operand" "=r")
        (ashift:SI (zero_extend:SI (match_operand:QI 1 "msp430_general_dst_operand" "rm"))
...
;; We started with: 
;; (set (reg:SI)
;;    (ashift:SI (zero_extend:SI (subreg:HI (mem/c:QI (symbol_ref:PSI)) 0))
;;        (const_int 2)))
;; So we are taking a char and shifting it and putting the result in 2 registers.
...
;; We started with:
;; (set (reg:SI)
;;    (ashift:SI (zero_extend:SI (sign_extend:PSI (subreg:HI (mem/c:QI (symbol_ref:PSI)) 0)))
;;        (const_int 2 [0x2])))
...
;; We started with:
;; (set (reg:SI)
;;    (zero_extend:SI (sign_extend:PSI (subreg:HI (mem/c:QI (symbol_ref:PSI)) 0))))
(define_insn ""
  [(set (match_operand:SI 0 "register_operand" "=r")
...
;; We started with:
;; (set (reg:PSI)
;;    (ashift:PSI (sign_extend:PSI (subreg:HI (mem/c:QI (symbol_ref:PSI)) 0))
;;        (const_int 2)))
(define_insn ""
  [(set (match_operand:PSI 0 "register_operand" "=r")
...
(define_insn "a<shift_insn>hi3_430"
...
;; All valid HI and PSI constant 430X left shifts
...
;; lshr<mode>3
;; i.e. logical shift right, or "srli" in mspabi speak
;; * For constant shift amounts, !msp430x, use the helper function unless the
;;   shift amount is XXX (decide based on insn length).
;; * For constant shift amount, msp430x, use either RRAM for shift amount between
;;   1 and 4, or RRAX with RPT otherwise.
;; * For shift amount in a register, always use __mspabi_srli
;;
;; FIXME: Based on these rules we should be able to condense all the shifts of
;; this type into fewer define_expand/define_insn.
    ...
    if (msp430_do_not_relax_short_jumps () && get_attr_length (insn) > 6)
      {
	    ...
	    (msp430_430x_insn_not_required (operands[1], operands[2]) ?
	     \"CMP.W\t%2, %1 { J%r0 1f { BRA #%l3 { 1:\" :

...
void bfd_elf_check_loc_metasyms (bfd *abfd, struct bfd_link_info *link_info);

bfd_boolean bfd_elf_add_metasym (bfd *abfd,
				 bfd *ibfd,
				 asymbol *sym,
				 Elf_Internal_Sym *elfsym,
				 Elf_Internal_SymMetaInfo *smi);

bfd_boolean bfd_elf_copy_metasyms (bfd *obfd, bfd *ibfd);

bfd_boolean bfd_elf_create_locsym_tables (bfd *ibfd, bfd *obfd);
...
/* Convenience macro for iterating over every metasym in the metasym hash
   table.  I must be an unsigned int, HASH_ENTRY is "struct bfd_hash_entry *"
   and MSYM_HASH_ENTRY is "struct bfd_elf_metasym_hash_entry *".
   FIXME - using a break statement with the macro only breaks out of the inner
   loop.
   FIXME can we merge these all into a single for loop using , separators? */
#define FOR_MSYM_IN_MSYM_HASH_TABLE(ABFD, I, HASH_ENTRY, MSYM_HASH_ENTRY)	\
  unsigned int I;								\
  struct bfd_hash_entry *HASH_ENTRY = NULL;					\
  struct bfd_elf_metasym_hash_entry *MSYM_HASH_ENTRY = NULL;			\
...
struct elf_size_info {
  ...
  unsigned char sizeof_rel, sizeof_rela, sizeof_sym, sizeof_metasym, sizeof_dyn, sizeof_note;
...
#define MAX_INT_RELS_PER_EXT_REL 3
  ...
  bfd_boolean (*swap_metasym_in)
    (bfd *, const void *, Elf_Internal_SymMetaInfo *);
  void (*swap_metasym_out)
    (bfd *, const Elf_Internal_SymMetaInfo *, void *);
...
struct bfd_elf_metasym_hash_table
{
  ...
  char *symtab_hash;
  /* Since we can have multiple metasyms with the same name, we can't just use
     bfd_hash_table.count.  */
  unsigned int count;
};
...
#define elf_metasym_count(bfd)	(elf_tdata(bfd) -> metasym_hash_table.count)
...
extern void bfd_elf32_swap_symbol_out
  (bfd *, const Elf_Internal_Sym *, void *, void *);
extern bfd_boolean bfd_elf32_swap_metasym_in
  (bfd *, const void *, Elf_Internal_SymMetaInfo *);
extern void bfd_elf32_swap_metasym_out
  (bfd *, const Elf_Internal_SymMetaInfo *, void *);
...
extern void bfd_elf64_swap_symbol_out
  (bfd *, const Elf_Internal_Sym *, void *, void *);
extern bfd_boolean bfd_elf64_swap_metasym_in
  (bfd *, const void *, Elf_Internal_SymMetaInfo *);
extern void bfd_elf64_swap_metasym_out
  (bfd *, const Elf_Internal_SymMetaInfo *, void *);

...
#define Elf_External_SymMetaInfo	NAME(Elf,External_SymMetaInfo)
...
#define elf_swap_metasym_in		NAME(bfd_elf,swap_metasym_in)
#define elf_swap_metasym_out		NAME(bfd_elf,swap_metasym_out)
...
#define ELF_SMI_INFO(X,Y)	ELF64_SMI_INFO(X,Y)
#define ELF_SMI_SYM(X)	ELF64_SMI_SYM(X)
#define ELF_SMI_KIND(X)	ELF64_SMI_KIND(X)
...
#define ELF_SMI_INFO(X,Y)	ELF32_SMI_INFO(X,Y)
#define ELF_SMI_SYM(X)	ELF32_SMI_SYM(X)
#define ELF_SMI_KIND(X)	ELF32_SMI_KIND(X)
...
/* Translate an ELF symbol meta-information entry in external format into an
   ELF symbol meta-information entry in internal format.  */
bfd_boolean
elf_swap_metasym_in (bfd *abfd,
		    const void *psrc,
		    Elf_Internal_SymMetaInfo *dst)
{
  const Elf_External_SymMetaInfo *src = (const Elf_External_SymMetaInfo *) psrc;
  bfd_vma tmp = H_GET_WORD (abfd, src->smi_info);
  dst->index = ELF_SMI_SYM (tmp);
  dst->kind = ELF_SMI_KIND (tmp);
  dst->value = H_GET_WORD (abfd, src->smi_value);
  return TRUE;
}

/* Translate an ELF symbol meta-information entry in internal format into an
   ELF symbol meta-information entry in external format.  */
void
elf_swap_metasym_out (bfd *abfd,
		     const Elf_Internal_SymMetaInfo *src,
		     void *cdst)
{
  Elf_External_SymMetaInfo *dst = (Elf_External_SymMetaInfo *) cdst;
  H_PUT_WORD (abfd, ELF_SMI_INFO (src->index, src->kind), dst->smi_info);
  H_PUT_WORD (abfd, src->value, dst->smi_value);
}
...
const struct elf_size_info NAME(_bfd_elf,size_info) = {
  ...
  sizeof (Elf_External_SymMetaInfo),

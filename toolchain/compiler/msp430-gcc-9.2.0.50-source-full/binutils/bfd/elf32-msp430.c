			  ...
			  bfd_boolean * again)
{
  ...
  *again = FALSE;
  ...
  /* Don't relax inside input BFDs which contain a .gcc_except_table section.
     The symbol diff calculation inside .uleb128 directives generated by GCC
     gets calculated by the assembler, but if the linker later relaxes in that
     section, the calculation can become invalidated.
     FIXME: Ideally we would have relocations in .gcc_except_table which the
     linker would then calculate and write out as .uleb128.  */
  if (bfd_get_section_by_name (abfd, ".gcc_except_table"))
    return TRUE;

...
error_free_dyn:
  ...
  if (elf_tdata (abfd)->symtabmeta_hdr.sh_size > 0
      ...
      if (symtab_hdr->contents == NULL && symtab_hdr->sh_info != 0)
	{
      ...
      if (!bfd_elf_slurp_symtab_meta (abfd, info->output_bfd, sympp, symcount, info))
	{
			      ...
			      long idx)
{
  ...
  for (kind = SMK_START; kind < SMK_END; kind++)
    {
	...
	bfd_elf_metasym_hash_lookup (&elf_tdata(obfd)->metasym_hash_table,
				     string, isym, ibfd->id, kind, FALSE, FALSE);
      if (h)
	h->msym.index = idx;
    }
}
  ...
  if (bfd_get_symcount (abfd) > 0 && elf_metasym_count (abfd) > 0)
    {
      ...
      bfd_elf_check_loc_metasyms (flinfo.output_bfd, info);

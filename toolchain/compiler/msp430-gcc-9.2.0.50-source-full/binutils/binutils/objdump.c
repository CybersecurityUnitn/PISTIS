  ...
  FOR_MSYM_IN_MSYM_HASH_TABLE (abfd, i, hash_entry, msym)
    {
      ...
      printf ("%d:\t", ii);
      /* Col width: 16 chars.  */
      printf ("%s\t", (msym->msym.kind == SMK_RETAIN ? "SMK_RETAIN"
		       : "SMK_LOCATION"));
      /* Col width: 16 chars.  */
      printf ("0x%lx%s", msym->msym.value, (msym->msym.value > 0xFFFFFFFF ? "\t"
				       : "\t\t"));
      /* Col width: 8 chars.  */
      printf ("%ld\t", msym->msym.index + 1);
...
dump_bfd (bfd *abfd, bfd_boolean is_mainfile)
{
  ...
  if (dump_symtab_meta && syms != NULL)
      ...
      if (bfd_get_flavour (abfd) != bfd_target_elf_flavour)
	{
      ...
      else if (!bfd_elf_slurp_symtab_meta (abfd, NULL, syms, symcount, NULL))
	{

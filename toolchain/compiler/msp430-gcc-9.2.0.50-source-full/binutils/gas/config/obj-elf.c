...
obj_elf_sym_meta_info (int arg ATTRIBUTE_UNUSED)
{
  ...
  symbolS *sym;
  asymbol *bfdsym;
  /* We're not going to initialize the index field.  */
  Elf_Internal_SymMetaInfo smi = {0, 0, 0};

  /* Read the name of the symbol this meta-information entry relates to.  */
  c = get_symbol_name (&sym_name);
  if (symbol_find (sym_name) == NULL)
    {
      as_bad (_("symbol \"%s\" does not exist or .sym_meta_info directive "
		"before symbol declaration"), sym_name);
  ...
  *input_line_pointer = c;
  ...
  if (!is_end_of_line[(unsigned char)*input_line_pointer])
    {
      ...
      smi.value = get_absolute_expression ();
      switch (smi.kind)
	{
	case SMK_RETAIN:
	  if (smi.value > 1)
  ...
  bfd_elf_add_metasym (stdoutput, NULL, bfdsym, NULL, &smi);
}

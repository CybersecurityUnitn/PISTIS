...
void print(const struct sudoku *a)
{
    ...
    for (i = 0; i < a->N1; i++)
        for (j = 0; j < a->N1; j++)
            printf("%i %c", a->a[i * a->N1 + j],(j == a->N1 - 1 ? '\n' : ' '));
}
...
bool solve(struct sudoku *a)
{
    ...
    { /* try all possible values 1 thru N */
        ...
        if (OK(a, i, j, v))
        {
            ...
            //printf("%sBacktrack (%d,%d) <- %i (counter=%i)\n",prefix, i, j, a->a[i * a->N1 + j], counter);
...
bool test()
{
    ...
    struct sudoku a;
    a.N1 = 9; a.N2 = 3; a.a = test_array;

    if(!solve(&a)){  // ensure that solution is obtained
        return false;
    }
    ...
    for (int i = 0; i < a.N1; i++)
        for (int j = 0; j < a.N1; j++)
            result*= (a.a[i * a.N1 + j] == expected[i * a.N1 + j]);
    return result;
...
int main()
{
    ...
    scanf("%" SCNu8, &(a.N1));
    a.a = (uint8_t *)malloc(a.N1 * a.N1 * sizeof(uint8_t));
    a.N2 = (uint8_t)sqrt(a.N1);

    for (int i = 0; i < a.N1; i++)
        for (int j = 0; j < a.N1; j++) scanf("%" SCNu8, &(a.a[i * a.N1 + j]));
